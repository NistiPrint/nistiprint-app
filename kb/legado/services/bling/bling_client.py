...
class BlingClient:
    ...
    def __init__(self, account_data):
        ...
        # Converter expires_in para int se vier como string do Firestore
    ...
    def create_client(**kwargs):
        ...
        if 'cnpj' in provided_keys:
            ...
            # Busca usando where query no campo 'cnpj'
            account_doc = None
            try:
                docs = accounts_collection.where('cnpj', '==', cnpj).limit(1).stream()
                for doc in docs:
                    account_doc = doc
                    break
                print(f"🔍 DEBUG: Document encontrado para CNPJ '{cnpj}': exists={account_doc.exists if account_doc else False}")
            ...
            for doc in query.stream():
                ...
                if ('account_name' in provided_keys and doc_data.get('account_name') == kwargs['account_name']) or \
                   ('client_id' in provided_keys and doc_data.get('client_id') == kwargs['client_id']):
                    account_doc = doc
                    print(f"🔍 DEBUG: Document encontrado por account_name/client_id: exists={account_doc.exists}")
                    break
            else:
                print(f"🔍 DEBUG: Nenhum documento encontrado para os critérios alternativos")
                account_doc = None

        if not account_doc or not account_doc.exists:
        ...
        account_data['id'] = account_doc.id  # Salvar o ID do documento

        print(f"✅ DEBUG: Conta encontrada! Document ID: '{account_doc.id}', Dados: account_name='{account_data.get('account_name','N/A')}'")
    ...
    def _refresh_token(self):
        ...
        if not self.refresh_token or len(str(self.refresh_token).strip()) == 0:
            ...
            if 'access_token' in response_data:
                ...
                if account_id:
                    try:
                        ...
                        firestore_client.collection('bling_accounts').document(account_id).update(update_data)
                        print(f"✅ Tokens atualizados no Firestore para conta {account_id}")
                    except Exception as e:
                        print(f"⚠️  Falha ao atualizar tokens no Firestore: {e}")
    ...
    @staticmethod
    def list_accounts():
        """Lista todas as contas Bling integradas do Firestore.

        Returns:
            list: Lista de contas com informações básicas
        """
        try:
            docs = firestore_client.collection('bling_accounts').stream()
            accounts = []
            ...
            for doc in docs:
                ...
                account_info = {
                    ...
                    'cnpj': account_data.get('cnpj', 'N/A'),
                    'platform': account_data.get('platform', 'N/A'),
                    'client_id': BlingClient._mask_sensitive_data(account_data.get('client_id', '')),
                    'token_status': 'ACTIVE' if account_data.get('access_token') else 'INACTIVE',
