...
def fix_combo_vacina(miolo):
    if 'CMB_ELOA' in miolo or 'CMB_AURORA' in miolo:
        return 'CMBMNA'
...
def process_string(item):
    ...
    if 'VACMNO_MCP6_BBB' in item['codigo']:
    ...
    if "PTDVOV_L&C2_BRB" in item['codigo']:
        return 'CEREJA PERSONALIZADO'
    
    if "VACMNO_VFOS1_BBB" in item['codigo']:
        return 'FOSCO VERDE ESCURO'
    
    if "VACMNO_VFOS2_BBB" in item['codigo']:
        return 'FOSCO VERDE'
    
    if "VACMNO_VFOS3_BBB" in item['codigo']:
        return 'FOSCO VERDE CLARO'
