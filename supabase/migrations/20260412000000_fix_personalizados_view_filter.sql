-- ============================================
-- Migração: Expandir filtro de origem em view_vendas_personalizadas_v3
-- Data: 2026-04-12
-- Objetivo: Incluir 'MARKETPLACE' no filtro de origem para incluir
--           pedidos que não são estritamente SHOPEE ou BLING
-- ============================================
...
SELECT
    ...
    (("v_shopee"."dados_brutos" -> 'informacoes_comprador'::"text") ->> 'username'::"text") AS "buyer_username",
    (EXISTS (
        SELECT 1
        FROM "public"."mensagem_chat_shopee" "mcs"
        WHERE (
            ("mcs"."from_user_name" = (("v_shopee"."dados_brutos" -> 'informacoes_comprador'::"text") ->> 'username'::"text"))
            OR ("mcs"."to_user_name" = (("v_shopee"."dados_brutos" -> 'informacoes_comprador'::"text") ->> 'username'::"text"))
        )
