-- Migration: Add marketplace information to list_pedidos_filtrados
-- Data: 2026-04-27
--
-- Objetivo: Atualizar list_pedidos_filtrados para retornar informações de marketplace
--           através de installed_integrations e integration_modules
--           Mantendo compatibilidade com canal_venda legado

-- Remover versão antiga
DROP FUNCTION IF EXISTS public.list_pedidos_filtrados(
    INTEGER, INTEGER, BOOLEAN, BOOLEAN, BOOLEAN, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, INTEGER
);

-- Criar função com marketplace information
...
BEGIN
    ...
    SELECT
        ...
        -- Cores por marketplace (Shopee: #EE4D2D, Mercado Livre: #FFF159, Amazon: #FF9900, Shein: #FF6B6B)
        CASE 
    ...
    LEFT JOIN installed_integrations ii ON p.marketplace_integration_id = ii.id
    LEFT JOIN integration_modules im ON ii.module_id = im.id
    ...
    WHERE
        ...
        AND (
            p_canal_venda_id IS NULL OR 
            p.canal_venda_id = p_canal_venda_id OR
            p.marketplace_integration_id = p_canal_venda_id
        )
        AND (
...
'Retorna lista de pedidos com filtros avançados (incluindo marketplace information), ordenada por numero_pedido.';
