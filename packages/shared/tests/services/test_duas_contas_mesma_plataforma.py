...
class InstalacaoNovaNaoNasceComAplicativoErradoTest(unittest.TestCase):
    """O carimbo errado e pior que carimbo nenhum.

    `install_module` usava `get_default_profile`, que devolve o primeiro
    profile ativo do modulo quando nenhum esta marcado como padrao. Com dois
    aplicativos ativos, a instalacao nova nascia apontando para um deles por
    ordem de retorno do banco — e a guarda de ambiguidade do resolvedor nunca
    dispara nesse caso, porque o vinculo existe; ele so aponta para a conta
    errada.
    """
    ...
    def setUp(self):
        ...
        self.service = installed_integration_service

    def _resolver(self, escolha, profiles):
        from nistiprint_shared.services import installed_integration_service as mod
        with patch.object(
            mod.integration_app_profile_service, "list_profiles", return_value=profiles
        ), patch.object(
            mod.integration_app_profile_service,
            "get_profile",
            side_effect=lambda pid: {5: PROFILE_A, 6: PROFILE_B}.get(pid),
        ):
            return self.service._resolve_install_app_profile_id("mercadolivre", escolha)

    def test_com_dois_aplicativos_e_sem_escolha_nasce_sem_vinculo(self):
        self.assertIsNone(self._resolver(None, [PROFILE_A, PROFILE_B]))

    def test_escolha_explicita_vence(self):
        self.assertEqual(self._resolver(6, [PROFILE_A, PROFILE_B]), 6)

    def test_aplicativo_unico_e_atribuido_sem_ambiguidade(self):
        self.assertEqual(self._resolver(None, [PROFILE_A]), 5)

    def test_aplicativo_de_outro_modulo_e_recusado(self):
        from nistiprint_shared.services import installed_integration_service as mod
        outro = {"id": 1, "module_id": "shopee", "is_active": True}
        with patch.object(
            mod.integration_app_profile_service, "get_profile", return_value=outro
        ):
            with self.assertRaises(ValueError):
                self.service._resolve_install_app_profile_id("mercadolivre", 1)
