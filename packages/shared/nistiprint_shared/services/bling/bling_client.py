...
class BlingClient:
    ...
    def __init__(self, account_data):
        ...
        if created_at_raw:
            if isinstance(created_at_raw, str):
                try:
                    self.created_at = datetime.datetime.fromisoformat(created_at_raw.replace('Z', '+00:00'))
                except ValueError:
                    self.created_at = datetime.datetime.now(datetime.timezone.utc)
            else:
                self.created_at = created_at_raw
        else:
            self.created_at = datetime.datetime.now(datetime.timezone.utc)

        # Timestamp da última atualização do token (usado para calcular expiração)
        updated_at_raw = account_data.get('updated_at')
        if updated_at_raw:
        ...
        # Uso defensivo de .lower() para evitar AttributeError se platform_name for None
        self.platform = str(account_data.get('platform_name') or '').lower()
    ...
    def create_client_from_integration(integration):
        """Cria uma instância de BlingClient a partir de um objeto InstalledIntegration.
        
        Args:
            integration (InstalledIntegration|dict): Objeto da integração instalada.
        """
    ...
    def create_client(**kwargs):
        ...
        if not provided_keys:
            raise ValueError("Identificador (cnpj, account_name ou client_id) obrigatório.")

        # Busca na tabela mestre de integrações
        query = supabase_db.table('installed_integrations').select("*").eq('module_id', 'bling').eq('is_active', True)
        
        if 'cnpj' in kwargs:
            cnpj = str(kwargs['cnpj']).strip()
            # Filtro em coluna JSONB para CNPJ
            res = query.contains('config', {'cnpj': cnpj}).execute()
        elif 'account_name' in kwargs:
            res = query.eq('instance_name', kwargs['account_name']).execute()
        elif 'client_id' in kwargs:
            res = query.contains('credentials', {'client_id': kwargs['client_id']}).execute()

        if not res.data:
            raise ValueError(f"Integração Bling não encontrada para: {kwargs}")
    ...
    def create_client_for_platform(platform_name, instance_name: str = None, channel_id: int = None, function_name: str = 'ORDER_IMPORT'):
        ...
        # Normalização robusta para busca em constants: remove espaços e converte para minúsculo
        platform_key = str(platform_name).lower().replace(' ', '')
        cnpj = PLATFORM_X_CNPJ.get(platform_key)
        if cnpj:
            print(f"ℹ️ Fallback para constants: Plataforma '{platform_name}' (chave: '{platform_key}') -> CNPJ '{cnpj}'")
    ...
    def _refresh_token(self):
        ...
        try:
            ...
            if 'access_token' in response_data:
                ...
                # Atualizar dados no Supabase (Tabela mestre installed_integrations)
                ...
                if account_id:
                    try:
                        update_data = {
                            ...
                            'refresh_token': self.refresh_token,
                            'expires_at': (datetime.datetime.utcnow() + datetime.timedelta(seconds=self.expires_in)).isoformat(),
                            'updated_at': datetime.datetime.utcnow().isoformat()
                        }
                        
                        # Atualizar no Supabase
                        supabase_db.table('installed_integrations').update(update_data).eq('id', account_id).execute()
                        print(f"✅ Tokens renovados no Supabase para conta {account_id}")
    ...
    def list_accounts():
        ...
        """
        try:
            response = supabase_db.table('contas_bling').select("*").execute()
            accounts = []
