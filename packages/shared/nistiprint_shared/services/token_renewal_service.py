...
class TokenRenewalService:
    ...
    def renew_mercadolivre_tokens_expiring_soon(self, expiry_threshold: timedelta = timedelta(hours=1)) -> dict:
        logger.info("Iniciando renovacao agendada de tokens Mercado Livre ativos.")

        response = supabase_db.client.table('installed_integrations') \
            .select('*') \
            .eq('module_id', 'mercadolivre') \
            .eq('is_active', True) \
            .execute()

        integrations = response.data or []
        if not integrations:
            logger.info("Nenhuma integracao Mercado Livre ativa encontrada.")
            return {
                'status': 'SUCCESS',
                'message': 'Nenhuma integracao Mercado Livre ativa encontrada',
                'total': 0,
                'renewed': 0,
                'failed': 0,
                'skipped': 0
        ...
        for integration in integrations:
            ...
            logger.info("Renovando token Mercado Livre para %s.", instance_name)
        ...
        result = {
            'status': 'SUCCESS',
            'message': 'Renovacao de tokens Mercado Livre concluida',
            'total': len(integrations),
            'renewed': renewed_count,
            'failed': failed_count,
            'skipped': skipped_count
