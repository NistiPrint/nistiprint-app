...
def setup_mock_query_interface():
    ...
    try:
        from nistiprint_shared.models.venda import Venda
    except ImportError:
        # Venda model might not exist yet
        Venda = None

    try:
        from nistiprint_shared.models.ordem_producao import OrdemProducao
    except ImportError:
        # OrdemProducao model might not exist yet
        OrdemProducao = None

    try:
        from nistiprint_shared.models.ordem_compra import OrdemCompra
    except ImportError:
        # OrdemCompra model might not exist yet
        OrdemCompra = None

    try:
        from nistiprint_shared.models.tag import Tag
    except ImportError:
        # Tag model might not exist yet
        Tag = None

    try:
        from nistiprint_shared.models.unidade_medida import UnidadeMedida
    except ImportError:
        # UnidadeMedida model might not exist yet
        UnidadeMedida = None

    try:
        from nistiprint_shared.models.plataforma import Plataforma
    except ImportError:
        # Plataforma model might not exist yet
        Plataforma = None

    try:
        from nistiprint_shared.models.canal_venda import CanalVenda
    except ImportError:
        # CanalVenda model might not exist yet
        CanalVenda = None

    try:
        from nistiprint_shared.models.categoria import Categoria
    except ImportError:
        # Categoria model might not exist yet
        Categoria = None

    try:
        from nistiprint_shared.models.daily_production_log import DailyProductionLog
    except ImportError:
        # DailyProductionLog model might not exist yet
        DailyProductionLog = None

    try:
        from nistiprint_shared.models.demanda_producao import DemandaProducao
    except ImportError:
        # DemandaProducao model might not exist yet
        DemandaProducao = None

    try:
        from nistiprint_shared.models.deposito import Deposito
    except ImportError:
        # Deposito model might not exist yet
        Deposito = None

    try:
        from nistiprint_shared.models.estoque_atual import EstoqueAtual
    except ImportError:
        # EstoqueAtual model might not exist yet
        EstoqueAtual = None

    try:
        from nistiprint_shared.models.recurso_produtivo import RecursoProdutivo
    except ImportError:
        # RecursoProdutivo model might not exist yet
        RecursoProdutivo = None

    try:
        from nistiprint_shared.models.notificacao import Notificacao
    except ImportError:
        # Notificacao model might not exist yet
        Notificacao = None

    try:
        from nistiprint_shared.models.configuracao_aplicacao import ConfiguracaoAplicacao
    except ImportError:
        # ConfiguracaoAplicacao model might not exist yet
        ConfiguracaoAplicacao = None

    try:
        from nistiprint_shared.models.fornecedor import Fornecedor
    except ImportError:
    ...
    try:
        from nistiprint_shared.models.produto_externo import ProdutoExterno
    except ImportError:
        ProdutoExterno = None
    ...
    try:
        from nistiprint_shared.models.system_events_log import SystemEventsLog
    except ImportError:
