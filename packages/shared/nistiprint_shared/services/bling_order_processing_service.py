...
class BlingOrderProcessingService:
    ...
    and saving valid orders to the database using the new refactored model (Parte A).
    """

    def __init__(self):
        self.supabase = supabase_db
        self._status_mapping_cache = {}
        # IDs de instâncias Bling conhecidas (hardcoded conforme plano 4.3 - a ser movido para config futuramente)
        self.BLING_ANTIGA_CNPJ = "13597" 
        self.BLING_NOVA_CNPJ = "54533"
        self._bling_antiga_account = None
        self._bling_nova_account = None

    @property
    def bling_antiga_account(self):
        if not self._bling_antiga_account:
            res = self.supabase.table('installed_integrations') \
                .select('*') \
                .ilike('config->>cnpj_identificador', f'%{self.BLING_ANTIGA_CNPJ}%') \
                .execute()
            if res.data:
                self._bling_antiga_account = res.data[0]
            else:
                # Fallback para busca por nome se config não estiver preenchida
                res = self.supabase.table('installed_integrations') \
                    .select('*') \
                    .ilike('instance_name', '%antiga%') \
                    .execute()
                if res.data:
                    self._bling_antiga_account = res.data[0]
    ...
    def bling_nova_account(self):
        if not self._bling_nova_account:
            res = self.supabase.table('installed_integrations') \
                .select('*') \
                .ilike('config->>cnpj_identificador', f'%{self.BLING_NOVA_CNPJ}%') \
                .execute()
            if res.data:
                self._bling_nova_account = res.data[0]
            else:
                res = self.supabase.table('installed_integrations') \
                    .select('*') \
                    .ilike('instance_name', '%nova%') \
                    .execute()
                if res.data:
                    self._bling_nova_account = res.data[0]
        return self._bling_nova_account

    def _get_bling_order_details(self, order_id, api_key):
        """Fetches full order details from Bling API V3."""
    ...
    def _get_bling_order_by_shopee_id(self, shopee_order_id, api_key):
    ...
    def _get_bling_product_details(self, product_id, api_key):
    ...
    def _upsert_pedido_master(self, payload, **kwargs):
        ...
        situacao_id = payload.get('situacao', {}).get('id')
        if situacao_id:
            mapping_res = self.supabase.table('integration_status_mappings') \
                .select('situacao_pedido_id') \
                .eq('plataforma', 'bling') \
                .eq('status_externo_id', str(situacao_id)) \
                .maybe_single().execute()
            if mapping_res.data:
                data['situacao_pedido_id'] = mapping_res.data['situacao_pedido_id']
    ...
    def _save_order_to_db(self, payload, integracao_bling_id):
        ...
        if conn and conn.get('marketplace_integration_id'):
            ...
            try:
                ...
                # shopee_driver.get_order_detail espera order_sn_list
                shopee_details = shopee_driver.get_order_detail(
                    integration=integration_shopee,
                    order_sn_list=[order_sn]
                )
                
                if shopee_details and 'raw' in shopee_details:
                    shopee_data = shopee_details.get('raw')
    ...
    def process_webhook(self, webhook_payload: dict):
        ...
        if not self.bling_antiga_account:
            raise Exception("Bling account (antiga) not found.")
        ...
        # 1. Get full order details from Bling API
        api_key_antiga = self.bling_antiga_account.get('credentials', {}).get('access_token') or self.bling_antiga_account.get('access_token')
        ...
        if self.bling_nova_account:
            api_key_nova = self.bling_nova_account.get('credentials', {}).get('access_token') or self.bling_nova_account.get('access_token')
            shopee_order_id = full_order_data.get('numeroLoja')
            if shopee_order_id and api_key_nova:
