...
class ContaBlingService:
    ...
    def get_all(self):
        ...
        for row in response.data:
            ...
            # Map 'nome' to 'name' and 'account_name' for compatibility
            account['name'] = row.get('nome')
    ...
    def get_by_id(self, account_id: str):
        ...
        if response.data:
            ...
            account['name'] = account.get('nome')
            account['account_name'] = account.get('nome')
            return account
        return None
    ...
    def get_by_platform(self, platform: str):
        ...
        # Se não encontrar, tenta variações de case
        if not response.data:
            response = self.table.select("*").eq('platform_name', platform.lower()).execute()
        
        if not response.data:
            response = self.table.select("*").eq('platform_name', platform.capitalize()).execute()
